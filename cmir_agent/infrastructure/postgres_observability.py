...
class PostgresAgentRunRepository(AgentRunRepository):
    ...
    def start(
        ...
        *,
        ...
        """Create one per-email agent run."""
...
class PostgresWorkflowThreadRepository(WorkflowThreadRepository):
    ...
    def get_by_thread_id(self, thread_id: str) -> Optional[WorkflowThread]:
        with self._db.session() as session:
            row = session.scalar(
                select(WorkflowThreadORM).where(WorkflowThreadORM.thread_id == thread_id)
            )
        if row is None:
            return None
...
class PostgresPendingHumanActionRepository(PendingHumanActionRepository):
...
class PostgresAgentTraceRepository(AgentTraceRepository):
    """SQLAlchemy repository for agent_trace."""
...
class PostgresHITLStateRepository(HITLStateRepository):
    ...
    def apply_human_action(
        ...
        *,
        ...
        with self._db.session() as session:
            ...
            new_pending_action_id: Optional[int] = None
