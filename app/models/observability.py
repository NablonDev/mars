...
class AgentRunORM(Base):
    ...
    run_type: Mapped[str] = mapped_column(String(64), default="email_ingest")
...
class WorkflowThreadORM(Base):
    ...
    status: Mapped[str] = mapped_column(String(64), default="running")
    current_node: Mapped[str | None] = mapped_column(String(100))
    stage: Mapped[str] = mapped_column(String(64), default="INGESTING")
    cmir_status: Mapped[str | None] = mapped_column(String(64))
...
class PendingHumanActionORM(Base):
    ...
    thread_id: Mapped[str] = mapped_column(
        String(100), ForeignKey(f"{CMIR_SCHEMA}.workflow_threads.thread_id")
    )
    email_id: Mapped[UUID | None] = mapped_column(UUID_PK, ForeignKey(f"{CMIR_SCHEMA}.email_events.id"))
    po_line_id: Mapped[UUID | None] = mapped_column(UUID_PK, ForeignKey(f"{CMIR_SCHEMA}.po_lines.id"))
    interrupt_type: Mapped[str] = mapped_column(String(64))
