...
class AgentRunORM(Base):
    ...
    current_node: Mapped[str | None] = mapped_column(String(150))
    run_type: Mapped[str] = mapped_column(String(50), default="email_ingest")
...
class WorkflowThreadORM(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID_PK, primary_key=True, default=generate_uuid7)
    thread_id: Mapped[str] = mapped_column(String(150), unique=True)
...
class PendingHumanActionORM(Base):
    ...
    thread_id: Mapped[str] = mapped_column(
        String(150), ForeignKey(f"{CMIR_SCHEMA}.workflow_threads.thread_id")
    )
    email_id: Mapped[UUID | None] = mapped_column(UUID_PK, ForeignKey(f"{CMIR_SCHEMA}.email_events.id"))
    po_line_id: Mapped[UUID | None] = mapped_column(UUID_PK, ForeignKey(f"{CMIR_SCHEMA}.po_lines.id"))
