...
class Delivery(Base, TimestampMixin):
    ...
    ship_from_plant_id: Mapped[UUID | None] = mapped_column(
        UUID_PK, ForeignKey(f"{COMMON_SCHEMA}.plant.id"), nullable=True
    )
    ship_from_warehouse_id: Mapped[UUID | None] = mapped_column(
        UUID_PK, ForeignKey(f"{COMMON_SCHEMA}.warehouse.id"), nullable=True
    )
...
class DeliveryLine(Base, TimestampMixin):
    ...
    id: Mapped[UUID] = mapped_column(UUID_PK, primary_key=True, default=generate_uuid7)
    delivery_id: Mapped[UUID] = mapped_column(UUID_PK, ForeignKey(f"{COMMON_SCHEMA}.delivery.id"))
...
class Shipment(Base, TimestampMixin):
    ...
    carrier_id: Mapped[UUID | None] = mapped_column(
        UUID_PK, ForeignKey(f"{COMMON_SCHEMA}.carrier.id"), nullable=True
    )
