"""Repository for persisted fine-projection-summary generation jobs."""
...
def _to_dict(row: ProjectionSummary) -> dict:
    return {
        "id": row.id,
...
class FineProjectionSummaryRepository:
    ...
    def _find(
        ...
        return self._session.scalars(
            select(ProjectionSummary).where(
                ProjectionSummary.order_id == order_id,
                ProjectionSummary.as_of_date == as_of_date,
                ProjectionSummary.prompt_version == prompt_version,
            )
        ).first()
    ...
    def get_cached(
        ...
        row = self._session.scalars(
            select(ProjectionSummary).where(
                ...
                ProjectionSummary.as_of_date == as_of_date,
                ProjectionSummary.prompt_version == prompt_version,
                ProjectionSummary.status == SummaryStatus.READY,
            )
        ).first()
    ...
    def get_latest_ready_not_after(
        ...
        row = self._session.scalars(
            ...
            .order_by(ProjectionSummary.as_of_date.desc())
        ).first()
    ...
    def find_reusable(
        ...
        *,
        ...
        effective_source_date = func.coalesce(
            ProjectionSummary.source_as_of_date, ProjectionSummary.as_of_date
        )

        conditions = [
            ProjectionSummary.order_id == order_id,
            ProjectionSummary.prompt_version == prompt_version,
            ProjectionSummary.content_fingerprint == content_fingerprint,
            ProjectionSummary.status == SummaryStatus.READY,
        ...
        row = self._session.scalars(
            select(ProjectionSummary).where(*conditions).order_by(effective_source_date.desc())
        ).first()
