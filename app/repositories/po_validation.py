...
class PostgresPoLineRepository:
    ...
    def get(self, po_line_id: Any) -> dict[str, Any] | None:
        with self._db.session() as session:
    ...
    def update_status(self, po_line_id: Any, status: str) -> None:
        with self._db.session() as session:
            session.execute(update(PoLineORM).where(PoLineORM.id == po_line_id).values(status=status))
...
class PostgresMaterialMasterRepository:
    ...
    def find(self, sap_material_number: str, plant: str) -> MaterialMasterRecord | None:
        with self._db.session() as session:
            row = session.scalar(
...
class PostgresPoLineErrorRepository:
    ...
    def log(self, error: PoLineError) -> str:
        with self._db.session() as session:
