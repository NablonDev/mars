...
class ServiceBusJobQueue(JobDispatcher, JobSource):
    ...
    def __init__(
        ...
        *,
        ...
        self._max_wait_seconds = settings.service_bus_max_wait_seconds
    ...
    def _build_real_client(settings: Settings) -> tuple[Any, Callable[[str], Any]]:
        ...
        if not settings.service_bus_namespace:
            raise ValidationError(
                "JOB_QUEUE_BACKEND=service_bus requires SERVICE_BUS_NAMESPACE to be "
                "set (e.g. 'mars-fines.servicebus.windows.net')."
            )
    ...
    def claim_batch(self, worker_id: str, limit: int) -> list[ClaimedJob]:
        ...
        with self._lock:
            messages = list(
                self._receiver.receive_messages(max_message_count=limit)
            )
        ...
        for row in claimed_rows:
            ...
            claimed_jobs.append(
                claimed_job_from_row(row, receipt=primary)
            )
    ...
    def reclaim_stale(self, visibility_timeout_seconds: int) -> int:
        ...
        with self._database.session() as session:
            return JobQueueRepository(session).reclaim_stale(
