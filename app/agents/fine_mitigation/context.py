"""Typed contract for the data handed to the fine-mitigation-summary LLM call."""
...
class FineMitigationSummaryContext(BaseModel):
    order: OrderContext
    current_projection_date: date  # the day the mitigation_options below were ranked for
    current_total_expected_fine: float  # the ACCEPT baseline -- also mitigation_options[0].projected_fine_after when ACCEPT ranks first, but given explicitly so the model never has to infer which entry is the baseline
    stacking_mode: str
