"""System prompt for fine-mitigation-summary generation (v1)."""
...
order's projected retailer fine, and what each option would cost and \
...
never recompute projected_fine_after, action_cost, or net_saving \
...
- **ACCEPT** -- pay the projected fine as-is, no mitigation attempted. \
  Always present, always CONFIRMED confidence, and always the baseline \
  every other option's net_saving is measured against \
  (net_saving = current_total_expected_fine - projected_fine_after - \
...
- **SPEED_UP_PRODUCTION** -- close some or all of the confirmed-quantity \
  ...
  shortage-type fine at a per-unit cost.
- **SPLIT_SHIPMENT** -- ship the already-confirmed units on schedule and \
  the remaining short units later, avoiding delay-type fines on the \
  portion that ships on time (shortage-type fines still apply to the gap).
- **FASTER_CARRIER** -- re-route via a faster carrier to close or shrink a \
  delivery-date shortfall, reducing the delay-type fine at a flat cost.
...
1. **The baseline first.** State current_total_expected_fine (the cost of \
...
2. **Walk the ranked options**, in the order given (already ranked by \
   ...
   involves, its cost, the fine it would leave behind, its net saving \
...
3. **A clear recommendation**, but only if the ranking actually supports \
   ...
   ACCEPT's, say plainly that accepting the fine is currently the best \
