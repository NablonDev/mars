...
from app.schemas.cmir import CMIR, CMIR_CONTENT_FIELDS, EmailMessage, PendingHumanAction
...
# (see app.repositories.cmir.CMIRVersionConflict). The thread still closes out
# (consistent with approve/reject) rather than silently reopening for retry --
# the exact reviewer-facing retry UX is an open product question (see
# docs/memory.md), so this is the minimal, safe behavior: fail loud, don't guess.
...
class CMIRRunService:
    ...
    def __init__(
        ...
        *,
        ...
        email_repository: PostgresEmailRepository | None = None,
    ...
    def update_draft(
        ...
        *,
        ...
        existing_draft = snapshot.get("cmir", {})
        proposed = CMIR(**{**existing_draft, **fields})

        current = self._cmir_repository.get_current(
            proposed.customer_identity, proposed.target_customer_material_ref
        )
        merged, diff = merge_with_active(current, proposed)

        field_changes = {
            field: {"from": existing_draft.get(field, ""), "to": value} for field, value in fields.items()
        }
        validated = self._update_graph_cmir(
            thread_id,
            merged.model_dump(),
        ...
        try:
            new_action_id = self._hitl_state.apply_human_action(
                ...
                next_latest_snapshot={"cmir": updated_cmir, "existing_cmir": current, "diff": diff},
    ...
    def _update_graph_cmir(
        self,
        thread_id: str,
        cmir: dict[str, Any],
        *,
        existing_cmir: dict[str, Any] | None = None,
        cmir_diff: dict[str, Any] | None = None,
        cmir_version_token: int | None = None,
    ) -> CMIR:
        ...
        if hasattr(self._graph, "update_state"):
            self._graph.update_state(
                ...
                {
                    "cmir": validated.model_dump(),
                    "existing_cmir": existing_cmir,
                    "cmir_diff": cmir_diff or {},
                    "cmir_version_token": cmir_version_token,
                },
            )
        else:
