...
See docs/FINE_ENGINE.md for the placeholder-data posture this mirrors
(ANTICIPATED_SHORTFALL_PCT, DELAY_PROBABILITY_TABLE): all cost/cause inputs
here are mocked, since no real Mars data source for them exists yet.
"""
...
from app.services.fine_projection.shortage import shortfall_units_for_pricing


def _accept_option(projection: ProjectionResult) -> MitigationOption:
    return MitigationOption(
        action="ACCEPT",
        projected_fine_after=projection.total_expected_fine,
        action_cost=0.0,
        net_saving=0.0,
        risk_level="HIGH" if projection.total_expected_fine > 0 else "LOW",
        confidence="CONFIRMED",
        rationale="Pay the projected fine as-is -- always knowable, no mitigation attempted.",
    )


def _speed_up_production_option(
    snapshot: OrderSnapshot, rules: list[FineRule], projection: ProjectionResult, inputs: MitigationInputs
    ...
    projected_fine_after = project_order(hypothetical, rules, projection.stacking_mode).total_expected_fine
    action_cost = closable_units * inputs.capacity_boost_cost_per_unit
    net_saving = projection.total_expected_fine - projected_fine_after - action_cost
    ...
    return MitigationOption(
        ...
        projected_fine_after=round(projected_fine_after, 2),
...
def _split_shipment_option(
    snapshot: OrderSnapshot, projection: ProjectionResult, inputs: MitigationInputs
) -> MitigationOption | None:
    ...
    # shortage fine off the true confirmed_qty vs order_qty gap today, so
    ...
    if projection.stacking_mode == "MAX":
        projected_fine_after = max((v.expected_fine for v in shortage_only), default=0.0)
    else:
        projected_fine_after = sum(v.expected_fine for v in shortage_only)
    ...
    rationale = (
        ...
        f"{snapshot.order_qty - snapshot.confirmed_qty} units later -- avoids delay fines, "
        "shortage fines still apply."
    )
...
def evaluate_mitigation_options(
    ...
    options = [_accept_option(projection)]

    speed_up = _speed_up_production_option(snapshot, rules, projection, inputs)
    if speed_up is not None:
        options.append(speed_up)

    split_shipment = _split_shipment_option(snapshot, projection, inputs)
    if split_shipment is not None:
        options.append(split_shipment)

    faster_carrier = _faster_carrier_option(snapshot, rules, projection, inputs)
