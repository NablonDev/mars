...
class FineSeedingService:
    ...
    def _truncate_seeded_tables(self) -> None:
        ...
        mitigation_input/mitigation_option/mitigation_summary and every
        sales_order fact/outcome table (order_confirmation,
        production_schedule, shipment, demand_exception, actual_fine,
        projected_fine, projection_summary) must be cleared before
        sales_order itself; sales_order and fine_rule must both be cleared
        before the retailer/sku/location/carrier master data they
    ...
    def simulate_daily_run(self) -> list[dict]:
        ...
        day."""
        return fine_projection_seed.simulate_daily_run(self.orders, self.projection_service)
