"""
Seeds master/reference data and replays the four worked-example scenarios
day by day. This is the logic behind `POST /admin/seed-master-data` and
`POST /admin/simulate-daily-run` -- the "seed via API" and "demo/reset
button" requirements. `scripts/seed_master_data.py` and
`scripts/demo_daily_simulation.py` are thin HTTP clients that call these
two endpoints; the actual logic lives here exactly once.

Both operations are idempotent: safe to call repeatedly (e.g. to reset a
demo environment) without duplicate-key errors.

Simplification versus the original simulate_daily_run.py: every day in a
scenario writes a new confirmation/production-status/shipment fact row,
even if the value is unchanged from the day before. The old script only
wrote a row when the value actually changed. Projection accuracy is
identical either way -- `OrderRepository.build_snapshot` always selects
the latest fact "as of" the projection date, so redundant unchanged rows
in between are harmless -- and always-write is simpler to reason about
than tracking last-known-value per order/sku/location.
"""
...
from app.engine import DELAY_VIOLATION_TYPES, SHORTAGE_VIOLATION_TYPES, FineRule
from app.engine.scenario_data import (
    AMZ_RULES,
    WMT_RULES,
    amz1_days,
    amz2_days,
...
def _rule_to_seed_dict(rule: FineRule, retailer_id: str) -> dict:
    """Converts one of the canonical WMT_RULES/AMZ_RULES (the same
    FineRule objects the pure-engine tests run against, from
    app.engine.scenario_data) into the kwargs FineRuleRepository.add_rule
    expects. Deriving from there instead of hand-typing a second rule list
    here means the seeded mock data and the already-validated test
    numbers can never quietly drift apart into two copies of the same
    four rules."""
    return {
...
class SeedingService:
    ...
    def seed_master_data(self) -> dict:
        ...
        for r in _RULES:
            if r["rule_id"] not in existing_rules:
    ...
    def simulate_daily_run(self) -> list[dict]:
        ...
        runs the projection, and marks the order DELIVERED after its
        final day. Returns one summary dict per order.

        Known limitation, not fixed here: AMZ-778501 and AMZ-780112 share
        a plant/SKU (SKU-WHI20 @ LOC-COL) with overlapping date ranges
        (Aug 11-13), but the two scenarios were authored independently and
        assign that shared production line different, contradictory
        statuses on some of those overlapping days. A real plant can only
        have one true status per day -- these two demo narratives were
        never reconciled into one. The production_id suffix below makes
        both rows coexist without a primary-key collision, and
        OrderRepository.build_snapshot's tiebreaker (status_date, then
        production_id, both descending) makes "which one wins" for a
        shared day deterministic and reproducible -- but not
        domain-meaningful. Don't read anything into which scenario's
        status "won" on Aug 11-13; it's an artifact of insertion order,
        not a modeling decision. See docs/FINE_ENGINE.md "Open items."
        """
        summaries = []
        production_seq = 0
