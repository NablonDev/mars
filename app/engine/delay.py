"""
Delay probability + pricing. Modeled as an ETA: an expected ship date
plus an expected transit time produces an expected delivery date,
compared against what the retailer requires. See docs/FINE_ENGINE.md
"Delay probability" for the full table and rationale.
"""

from datetime import date, timedelta

from app.engine.models import AppointmentStatus, CalcType, FineRule, OrderSnapshot, ProductionStatus
...
def resolve_expected_ship_date(s: OrderSnapshot) -> date:
    """The 'ETA' logic: what's our current best estimate of the ship date?
    Checked in order of how specific/trustworthy the signal is:
      1. It already happened -- a fact, not an estimate.
      2. Caller supplied a real, specific updated estimate (e.g. an actual
         rescheduled DC appointment date).
      3. A dock appointment was actually missed -- a concrete execution event.
      4. Production status alone -- a generic, lower-confidence leading
         indicator. Only used when nothing more specific is available.
    """
...
def price_delay_fine(rule: FineRule, order_qty: int, unit_price: float) -> float:
    """Fine-if-realized is treated as a flat reference cost for the
    violation type, not scaled by the current buffer -- probability
    already carries the 'will it happen' judgment. See
    docs/FINE_ENGINE.md, "Pricing the fine"."""
