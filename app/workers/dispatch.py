...
domain modules this file imports from (`app/workers/fine_projection.py`,
`app/workers/fine_mitigation.py`). This split keeps the two fine
...
from app.workers.fine_mitigation import run_mitigation_summary
from app.workers.fine_projection import run_projection, run_summary
...
def execute_job(
    ...
    app/api/v1/fine_mitigation/mitigations.py).
    """
    if job.task_type == JobTaskType.ORDER_RUN:
        run_projection(job, database)
        run_summary(job, database, llm, heartbeat=heartbeat)
    elif job.task_type == JobTaskType.PROJECTION_SUMMARY_REGEN:
        run_summary(job, database, llm, heartbeat=heartbeat)
    elif job.task_type == JobTaskType.MITIGATION_SUMMARY_REGEN:
