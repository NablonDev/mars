...
response, which already carries every day's numbers.
...
def main() -> None:
    ...
    for scenario in resp.json()["scenarios"]:
        print(f"\n=== {scenario['order_id']} -- day-by-day projection trend ===")
        for day in scenario["days"]:
            print(
                f"  {day['projection_date']}  "
                f"shortage={day['shortage_probability'] * 100:5.1f}% (${day['shortage_fine']:>7,.2f})  "
                f"delay={day['delay_probability'] * 100:5.1f}% (${day['delay_fine']:>7,.2f})  "
                f"total=${day['total_expected_fine']:>8,.2f}   {day['note']}"
            )
