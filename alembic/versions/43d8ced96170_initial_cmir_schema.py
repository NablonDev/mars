...
Full re-squash of the entire `cmir`-schema migration history (previously
6 revisions interleaved with the `fines`-schema chain: 8209afe73fa4,
ce704d36b4f3, d9c2a745b810, e4b7c391a052, plus the cmir-relevant portions
of the two mixed-concern migrations -- see e803d9470f31 for the equivalent
`fines`-schema squash) into one clean migration, chained after the fines
squash -- mirroring how `cmir` originally joined the migration history
after `fines` (8209afe73fa4's down_revision was 5589e602eefa). Nothing
here actually depends on any `fines` table; this is a linear-history
choice, not a real dependency, made for the same reason the original chain
did it: one linear history is simpler to reason about than a branch.

No table renames in this schema (only `fines`-schema tables had the
`dim_`/`fact_` prefix). DDL derived from `Base.metadata` via `alembic
revision --autogenerate` against a truly empty Postgres database, then
hand-reviewed -- this also captures, in their final form and without
carrying forward the intermediate steps, everything the replaced chain
built up over time:
    - bounded String(n) columns instead of unbounded Text (ce704d36b4f3)
    - jsonb instead of plain json for the JSONB_OR_JSON-typed columns
      (ce704d36b4f3)
    - the created_at/updated_at/deleted_at triplet on every table except
      cmir_records, which uses its own SCD2 valid_from/valid_to/is_current
      instead (ce704d36b4f3)
    - FKs from agent_runs/hitl_actions to email_events/po_lines
      (ce704d36b4f3)
    - uuid (not integer) id columns on cmir_records, email_action_logs,
      hitl_actions, pending_human_actions, and agent_traces, and uuid (not
      integer) cmir_records.superseded_by_id / workflow_threads.
      pending_action_id (d9c2a745b810, e4b7c391a052)
...
def upgrade() -> None:
    ...
    op.create_table(
        "material_master",
        sa.Column("id", sa.Uuid(), nullable=False),
        sa.Column("sap_material_number", sa.String(length=50), nullable=False),
        ...
        sa.Column("follow_up_material_number", sa.String(length=100), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("po_line_number", sa.String(length=30), nullable=False),
        sa.Column("customer_id", sa.String(length=50), nullable=False),
        sa.Column("customer_material_code", sa.String(length=100), nullable=False),
        sa.Column("plant", sa.String(length=30), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("thread_id", sa.String(length=150), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("customer_identity", sa.String(length=150), nullable=False),
        sa.Column("material_identity", sa.String(length=150), nullable=False),
        sa.Column("intent_phrase", sa.Text(), nullable=False),
        sa.Column("existing_cmir_ref", sa.String(length=100), nullable=False),
        sa.Column("brand", sa.String(length=100), nullable=False),
        sa.Column("site", sa.String(length=50), nullable=False),
        sa.Column("target_grd_code", sa.String(length=100), nullable=False),
        sa.Column("target_customer_material_ref", sa.String(length=100), nullable=False),
        sa.Column("effective_date", sa.Date(), nullable=True),
        sa.Column("reason", sa.Text(), nullable=False),
        sa.Column("customer_identity_key", sa.String(length=150), nullable=False),
        sa.Column("target_customer_material_ref_key", sa.String(length=100), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("email_id", sa.Uuid(), nullable=True),
        sa.Column("po_line_id", sa.Uuid(), nullable=True),
        sa.Column("source_message_id", sa.String(length=300), nullable=True),
        sa.Column("sender", sa.String(length=320), nullable=True),
        sa.Column("subject", sa.Text(), nullable=True),
        sa.Column("status", sa.String(length=50), nullable=False),
        sa.Column("current_node", sa.String(length=150), nullable=True),
        sa.Column("stage", sa.String(length=50), nullable=False),
        sa.Column("cmir_status", sa.String(length=50), nullable=True),
        sa.Column(
            "latest_snapshot",
            sa.JSON().with_variant(postgresql.JSONB(astext_type=sa.Text()), "postgresql"),
            nullable=True,
