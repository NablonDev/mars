...
| `SERVICE_BUS_NAMESPACE` | `""` | both | FQDN, e.g. `mars-fines.servicebus.windows.net`. Only used when the backend is `service_bus` |
| `SERVICE_BUS_QUEUE_NAME` | `fine-projection-jobs` | both | |
| `SERVICE_BUS_MAX_WAIT_SECONDS` | `10` | worker | Receive long-poll window |
...
az containerapp update -g $RG -n mars-fines-api \
  --set-env-vars JOB_QUEUE_BACKEND=service_bus \
                 ...
                 SERVICE_BUS_QUEUE_NAME=fine-projection-jobs
