...
fine-summary endpoints (POST /orders/{order_id}/summary, POST
...
from app.api.dependencies import get_fine_summary_job_runner, get_llm_client
...
def _summary_job_items(db_session, order_id: str) -> list[JobItem]:
    return list(
        db_session.scalars(
            select(JobItem).where(JobItem.order_id == order_id, JobItem.task_type == "SUMMARY_REGEN")
        ).all()
    )
...
def test_cache_miss_creates_a_job_item_and_dispatches(seeded_client, db_session):
    ...
    assert len(items) == 1
    item = items[0]
    assert item.task_type == "SUMMARY_REGEN"
...
def test_background_job_marks_job_item_dead_on_failure(seeded_client, db_session):
    ...
    # The FAILED status is still visible on the fine_summary ledger too --
...
def test_lost_claim_does_not_execute_generation(database):
    ...
    with database.session() as session:
        ...
        item = repo.enqueue(run["id"], "ORD-LOST-CLAIM", date(2026, 8, 9), "SUMMARY_REGEN", max_attempts=5)
    ...
    run_job = get_fine_summary_job_runner(database=database, llm=_ExplodingLLMClient(), settings=Settings())

    # Must not raise, and must not touch FineSummaryService/order lookups
