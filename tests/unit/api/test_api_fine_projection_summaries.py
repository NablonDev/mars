"""Integration tests for the fine-projection-summary endpoints; TestClient runs BackgroundTasks synchronously before returning, so a PENDING job is already resolved by the time a later GET polls it."""
...
def test_fine_projection_summary_post_404s_for_unknown_order(seeded_client):
    _override_llm_client(seeded_client)

    resp = seeded_client.post("/api/v1/orders/NOPE/projection-summary", json={})

    assert resp.status_code == 404


def test_fine_projection_summary_post_422s_when_no_projection_exists_yet(seeded_client):
    """WMT-100234 is seeded but has no projection run in this test."""
    _override_llm_client(seeded_client)

    resp = seeded_client.post("/api/v1/orders/WMT-100234/projection-summary", json={})

    assert resp.status_code == 422


def test_fine_projection_summary_get_404s_for_unknown_order(seeded_client):
    resp = seeded_client.get("/api/v1/orders/NOPE/projection-summary")

    assert resp.status_code == 404


def test_fine_projection_summary_get_422s_when_no_projection_exists_yet(seeded_client):
    resp = seeded_client.get("/api/v1/orders/WMT-100234/projection-summary")

    assert resp.status_code == 422


def test_fine_projection_summary_get_404s_when_no_job_was_ever_scheduled(seeded_client):
    ...
    resp = seeded_client.get(
        "/api/v1/orders/WMT-100234/projection-summary", params={"as_of_date": "2026-08-09"}
    )

    assert resp.status_code == 404


def test_fine_projection_summary_202s_then_ready_on_poll_for_a_real_projected_order(seeded_client):
    run_resp = seeded_client.post(
        "/api/v1/orders/WMT-100234/projections",
        json={"projection_date": "2026-08-09"},
    )
...
def test_fine_projection_summary_422s_for_an_as_of_date_in_the_future(seeded_client):
...
def test_fine_projection_summary_422s_for_an_as_of_date_before_the_earliest_projection(seeded_client):
    run_resp = seeded_client.post(
        "/api/v1/orders/WMT-100234/projections",
        json={"projection_date": "2026-08-09"},
    )
...
def test_fine_projection_summary_force_regenerate_against_an_existing_ready_row_succeeds(seeded_client):
...
def test_fine_projection_summary_background_job_failure_surfaces_as_failed_status_not_a_raw_500(
    seeded_client,
):
    ...
    assert body["error_message"] == "Fine projection summary generation failed upstream"
