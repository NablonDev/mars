    ...
    def flush(self) -> None:
        ...
        if self._flush_count == self._fail_on_flush_number:
            raise IntegrityError(
                "INSERT INTO cmir_records ...", {}, Exception("duplicate key value violates unique constraint")
            )
        super().flush()
...
class SQLAlchemyRepositoryTests(unittest.TestCase):
    ...
    def test_get_current_returns_full_content_field_dict(self) -> None:
        ...
        for field_name in (
            "sender_type", "customer_identity", "material_identity", "intent_phrase",
            "existing_cmir_ref", "brand", "site", "target_grd_code",
            "target_customer_material_ref", "effective_date", "reason",
        ):
